...
export default function Item(props: props) {
    const { logo, title, actionText, contentText } = props;
    ...
    return (
        <article className="rounded-2xl transition-all border border-[#2c2c6c] hover:border-[#4db5ff66] hover:bg-transparent bg-[#2c2c6c] flex flex-col items-center p-5">
            ...
            <Link to="mailto:iburaksag@gmail.com" target="_blank" className="mt-3 text-[#4db5ff] text-xs leading-5 hover:text-white">{actionText}</Link>
        </article>
    )
}